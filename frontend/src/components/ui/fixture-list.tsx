...
import { columns } from "@/lib/columns/fixture-columns"
...
export function FixtureList({ data }: FixtureListProps) {
  const [status, setStatus] = useState("")
  const [author, setAuthor] = useState("")
  const [search, setSearch] = useState("")

  // Extract unique options from data
  const statusOptions = Array.from(new Set(data.map(f => f.status)))
  const authorOptions = Array.from(new Set(data.map(f => f.author)))

  // Filtered data
  const filtered = data.filter(f =>
    (status ? f.status === status : true) &&
    (author ? f.author === author : true) &&
